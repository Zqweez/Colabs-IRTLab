...
if false; then
  ...
  RAW_DIR="./original-NGS-data"
  OUT_DIR="./post_stari_fastqc_reports"
  
  # Create output directory if it doesn't exist
  mkdir -p "$OUT_DIR"
  
  # Run FastQC on all fastq.gz files in the input folder
  fastqc -t 4 -o "$OUT_DIR" "$RAW_DIR"/*.fq.gz
fi
```
...
filt_path <- file.path(path, "285-filtered")
...
# GreenGenesTaxM <- assignTaxonomy(seqtab.nochim,"silva_nr_v138_train_set.fa")
```
...
# Inspect which ASVs are almost identical
threshold <- 0.997
close_pairs <- which(sim_mat > threshold & sim_mat < 1, arr.ind=TRUE)
# remove duplicates (i,j) vs (j,i)
close_pairs <- close_pairs[close_pairs[,1] < close_pairs[,2], , drop=FALSE]

if (nrow(close_pairs)>0) {
  cat(dim(close_pairs)[1], 'ASVs with a similarity higher than', threshold)
  apply(close_pairs, 1, function(idx) {
    i <- idx[1]; j <- idx[2]
    cat(sprintf("%s ↔ %s : sim = %.3f\n",
                asv_ids[i], asv_ids[j], sim_mat[i,j]))
  })
} else {
  message("No ASV pairs exceed similarity threshold of ", threshold)
}
...
# Save
# write.csv(raw_ASV_counts, file = "ASVs-raw-counts-no-seq.csv")
