...
# Heatmap
ggplot(alignment_results, aes(ASV_row1, ASV_row2, fill = PID)) +
  ...
  for (i_similarity in loop_similarity) {
    ...
    for (i_sample in loop_samples) {
      ...
      if (show_taxa) {
        ...
        summarise(ClustRelAbundance = sum(RelativeAbundance), .groups = "drop") %>%
        add_count(ClusterID, wt = ClustRelAbundance,
                  name = "total_abundance_final") %>%
      ...
      if (show_taxa) {
        ...
        ggplot(cluster_df, aes(x = Sample, y = ClustRelAbundance, fill = ClusterID)) +
          geom_bar(stat = "identity", color = "grey30", linewidth = 0.2) +
          scale_y_continuous(name = "Relative Abundance",
            labels = scales::percent_format(),
            expand = c(0, 0) # Remove padding at the bottom of the bars
...
ggsave(sprintf("../../results/ngs/figures/Combined-all-samples-%s-%s-filter-%s.pdf", linkage, similarity, filter_level), width = 12, height = 5)
```
...
writeXStringSet(dna, filepath = "../../results/ngs/top_ASVs_per_taxa.fasta", format   = "fasta")
...
ggsave(sprintf("../../results/ngs/figures/Combined-all-samples-taxa-cluster-%s-filter-%s.pdf", cluster_taxa_level, filter_level), width = 12, height = 5)
...
ggsave(sprintf("../../results/ngs/figures/%s-only-complete-ASVs-filter-%s.pdf", filter_ASV, filter_level), width = 12, height = 5)
```
