...
#| include: false
process_sample <- "phe" #phe, nap, none
...
ggplot(cluster_df, aes(x = Sample, y = ClustRelAbundance, fill = Label)) +
  ...
  scale_fill_manual(name = sprintf("Species cluster"), values = final_palette) +
  guides(fill = guide_legend(
   override.aes = list(size = 3),   # icon / key size
   ncol = 2 
  )) +
...
```{r}
# Sphingomonadaceae Sphingobium czechense 
# Cupriavidus plantarum, metallidurans, necator
filter_ASV <- "Cupriavidus necator" 
taxa_filter <- "s" # s, g or f
taxa_filter_map <- if (taxa_filter == "s") "species" else if (taxa_filter == "g") "genus" else if (taxa_filter == "f") "family"
filter_level <- 0 # To reduce the number of ASVs only keep the most abundant

taxa_of_interest <- taxa_df %>%
  filter(.data[[taxa_filter]] == filter_ASV)
...
# Titles and stuff
title_txt <- sprintf("Community composition — ASVs in %s %s", taxa_filter_map, filter_ASV)
...
ggplot(ASV_plot_data, aes(x = Sample, y = ClustRelAbundance, fill = Label)) +
  ...
  guides(fill = guide_legend(
   override.aes = list(size = 2),   # icon / key size
   ncol = 2
  )) +
  labs(x = "Sample", title = title_txt, 
       subtitle = subtitle_txt, caption = caption_txt) +
...
ggsave(sprintf(paste0(folder_path, "figures/", process_sample, "-%s-only-none-complete-ASVs-filter-%s.pdf"), filter_ASV, filter_level), width = 12, height = 5)
```
...
early_start  <- 0   # first x-value of the EARLY window
late_start   <- 18  # first x-value of the LATE  window
window_size  <- 5   # how many consecutive points to sum in each window
...
# Calculate window abundance
fold_tbl <- cl_long %>%
  group_by(Replicate, Label) %>%
  summarise(
    early_abund = sum(
      ClustRelAbundance[DataPoint %in%
        seq(early_start, length.out = window_size, by = 1)]
    ),
    late_abund  = sum(
      ClustRelAbundance[DataPoint %in%
        seq(late_start,  length.out = window_size, by = 1)]
    ),
    fold_change = round(late_abund / early_abund, 2),
    .groups = "drop"
  )
clusters_big_jump <- fold_tbl %>%
  filter(fold_change >= fold_change_threshold) %>%
  count(Label) %>%
  filter(n == 3)
...
cluster_levels <- as.character(unique(plotting_clusters_df$Label))
n_clusters     <- length(cluster_levels)
# Colors using polychrome
set.seed(0)
anchor_pastel <- c("#FF0000", "#00FF00", "#0000FF")
...
# Titles and stuff
title_txt <- sprintf("Community composition")
subtitle_txt <- glue("For clusters with substantial abundance change")
caption_txt <- sprintf("Clusters with >%s fold change between point %s and %s, sliding window of %s", fold_change_threshold, early_start, late_start, window_size)
...
ggplot(plotting_clusters_df, aes(x = Sample, y = ClustRelAbundance, fill = Label)) +
  ...
  scale_fill_manual(name = "Species cluster", values = final_palette) +
  guides(fill = guide_legend(
   override.aes = list(size = 3),   # icon / key size
   ncol = 1
  )) +
...
ggsave(sprintf(paste0(folder_path, "figures/", process_sample, "-abundance-change-%s-between-%s-and-%s.pdf"), fold_change_threshold, early_start, late_start), width = 12, height = 5)
```
